...
public class TypeCheckVisitor implements ASTVisitor {
	...
	public Object visitIBinaryExpression(IBinaryExpression n, Object arg) throws Exception {
		...
		IType leftType = (IType) left.visit(this, arg);
		
		IExpression right = n.getRight();
		IType rightType = (IType) right.visit(this, arg);
		
		Kind op = n.getOp();
		
		if( (op==Kind.LT || op==Kind.GT || op==Kind.EQUALS || op==Kind.NOT_EQUALS ) && ( leftType.equals(rightType) ))
			n.setType(PrimitiveType__.booleanType);
		else if(op==Kind.PLUS && ( ( leftType.isInt() && rightType.isInt() ) || ( leftType.isString() && rightType.isString() ) || ( leftType.isList() && rightType.isList() ) ))
			n.setType(rightType);
		else if( (op==Kind.MINUS || op==Kind.TIMES || op==Kind.DIV ) && (( leftType.isInt() && rightType.isInt() )))
			n.setType(rightType);
		else if( (op==Kind.AND || op==Kind.OR) && ((leftType.isBoolean() && rightType.isBoolean())))
			n.setType(PrimitiveType__.booleanType);
		else check(false, n, "Illegal Binary Expression");
	...
	public Object visitIIfStatement(IIfStatement n, Object arg) throws Exception {
		...
		IType guardType = (IType) guardExpression.visit(this, arg);
	...
	public Object visitIReturnStatement(IReturnStatement n, Object arg) throws Exception {
		...
		/*
		 * arg is the IFunctionDeclaration in this case
		 */
		IExpression expression = n.getExpression();
		IType returnType = (IType) expression.visit(this, arg);
		
		/*
		 * Checking if the function return type matched with the expression return type
		 */
		IFunctionDeclaration functionDeclaration = (IFunctionDeclaration) arg;
		IType resultType = functionDeclaration.getResultType();
		
		if(!compatibleAssignmentTypes(returnType, resultType))
		{
			check(false, n, "The retrun type is not compatible with the result type in the function declared");
		}
	...
	public Object visitISwitchStatement(ISwitchStatement n, Object arg) throws Exception {
		...
		IType switchExpressionType = (IType) switchExpression.visit(this, arg);
		check(switchExpressionType.isBoolean() || switchExpressionType.isInt() || switchExpressionType.isString(), n, "Guard expression type not amongst boolean, string or int");
		
		List<IExpression> branchExpressions = n.getBranchExpressions();
		for(IExpression singleBranchExpression : branchExpressions)
		{
			IType singleBranchExpressionType = (IType) singleBranchExpression.visit(this, arg);
			check(switchExpressionType.equals(singleBranchExpressionType), n, "Guard expression type not in aligned with the case expression");
			check(isConstantExpression(singleBranchExpression),n,singleBranchExpression + "is not a constant");
		}
		
		for(IBlock singleBlock : n.getBlocks())
		{
			singleBlock.visit(this, arg);
		}
